...
class ShuffleNetV2(nn.Module):
    def __init__(
        ...
        if model_size == '0.05x':
            self._stage_out_channels = [16, 8, 16, 32, 64]
        elif model_size == '0.05x_extreme':
            self._stage_out_channels = [16, 8, 16, 32, 64]
            self.stage_repeats = [1, 1, 1]
        elif model_size == '0.1x':
            self._stage_out_channels = [16, 16, 32, 64, 256]
        elif model_size == '0.25x':
            self._stage_out_channels = [24, 24, 48, 96, 512]
    ...
    def _initialize_weights(self, pretrain=True):
        ...
        # 2. 尝试加载预训练权重（增加安全性检查）
        if pretrain:
            model_key = "shufflenetv2_{}".format(self.model_size)
            ...
            if model_key in model_urls:
                ...
                # 如果是 0.25x 这种自定义规格，字典里没 key，打印提示并跳过
                print("=> Warning: No pretrained model found for {}, using random init.".format(model_key))