...
class NanoDetPredictor:
    def __init__(self, cfg, model_path, device='cuda:0'):
        ...
        # --- 核心逻辑：过滤蒸馏模型权重，只保留学生模型 (model. 开头的键) ---
        new_state_dict = {}
        for k, v in state_dict.items():
            if k.startswith('model.'):
                # 去掉前缀 'model.' 以对齐单体模型结构
    ...
    def inference(self, img):
        ...
        # 强制缩放至 YML 定义的输入尺寸 (如 160x160 或 320x320)
        ...
        warp_matrix = np.array([
            [self.input_size[0] / w_orig, 0, 0],
            [0, self.input_size[1] / h_orig, 0],
            [0, 0, 1]
        ], dtype=np.float32)
        ...
        meta = {
            'img': img_tensor,
            'img_info': {'height': [h_orig], 'width': [w_orig], 'id': [0]},
...
def evaluate_nanodet_standard():
    ...
    print("\n" + "=" * 60)
    print(f"运行时逐层特征图内存拆解 (Activations):")
    total_fm_kb = 0
    for i, info in enumerate(predictor.feature_info):
        print(f"  Level {i} | 维度: {info['shape']} | 占用: {info['mem_kb']:.2f} KB")
        total_fm_kb += info['mem_kb']

    # 参数内存占用 (Params * 4 bytes)
    params_weight_kb = predictor.params_m * 1024 * 4
    grand_total_kb = params_weight_kb + total_fm_kb
    print(f"  特征图总计: {total_fm_kb:.2f} KB")
    print("=" * 60 + "\n")
    ...
    # ================= 最终报告展示 =================
    print("\n" + " NanoDet 100KB 极致轻量化全指标报告 ".center(60, "="))
    print(f" [内存占用分析]")
    print(f" 1. 学生模型参数量:".ljust(25) + f"{predictor.params_m:.6f} M")
    print(f" 2. 权重显存占用:".ljust(25) + f"{params_weight_kb:.2f} KB")
    print(f" 3. 特征图显存占用:".ljust(25) + f"{total_fm_kb:.2f} KB")
    print(f" >> 推理总消耗 (Total):".ljust(25) + f"{grand_total_kb:.2f} KB")
    print("-" * 60)
    print(f" [目标检测指标 - 框级]")
    ...
    print(f"结果已存入: {SAVE_DIR}")


if __name__ == '__main__':
    evaluate_nanodet_standard()